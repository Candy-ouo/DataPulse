export default function TopKpiBar({ data = [] }) {
  ...
  return (
    <div className="grid grid-cols-6 gap-[10px] w-full">
      {data.map((item, i) => (
        <div key={i} className={`rounded-[10px] px-[16px] py-[10px] flex items-center gap-[14px] transition-all duration-200 animate-fade-in ${i === 0 ? 'animate-ripple' : ''}`}
          style={{
            animationDelay: `${i * 0.08}s`,
            background: 'linear-gradient(135deg, rgba(0,212,255,0.07), rgba(0,180,216,0.02))',
